...
const MenuButton = ({ onClick, title, isActive, Icon, children }: Props) => {
  ...
  return (<Tooltip title={anchorEl ? "" : title} disableInteractive>
    <span>
      <FieldButton ref={ref} type="button" isactive={isActive} width="3rem">
        <FlexBox width="100%" height="100%" padding="0" gap="0">
          <Icon sx={{ width: "1rem" }} onClick={(e: React.MouseEvent<Element, MouseEvent>) => onClick(e, AutoCalculate.SUM)} />
          <ArrowDropDown sx={{ width: "1rem", height: "100%" }} onClick={openMenu} />
        </FlexBox>
      </FieldButton>
      {anchorEl && (<Menu
        anchorEl={anchorEl}
      ...
      >
        ...
        (<MenuItem value={calc} key={calc} onClick={(e) => handleSelect(e, calc)}>
          {capitalize(calc)}
        </MenuItem>)
        )}
      </Menu>)}
