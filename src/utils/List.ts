...
class List<T> extends Array<T> {
   ...
   * @returns {List}
   */
  subList(fromIndex: number | undefined, toIndex: number | undefined): List<T> {
    return this.slice(fromIndex, toIndex) as List<T>;
  }
...
class Triplet extends Array {
  ...
  toList<T>(): List<T> {
    return listOf<T>(this[0], this[1], this[2]);
  }
}
 ...
 * @returns {Boolean}
 */
function isFn(argument: unknown): boolean {
 ...
 * @returns {Boolean}
 */
function isString(argument: unknown): boolean {
  return typeof argument === "string" || argument instanceof String;
}

function listOf<T>(...args: T[]): List<T> {
  if (args.length === 1) {
    return new List<T>(1).fill(args[0]);
  }
